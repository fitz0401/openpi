...
@pytest.mark.parametrize(
    ("filename", "source_ids", "target_ids"),
    [
        ("libero_main_18source_22target.json", [4, 2, 9, 7, 6, 8], [5, 1, 3, 0]),
        ("libero_split_b_18source_22target.json", [1, 3, 5, 7, 8, 9], [0, 2, 4, 6]),
        ("libero_split_c_18source_22target.json", [0, 2, 3, 4, 5, 6], [1, 7, 8, 9]),
    ],
)
def test_formal_splits_keep_one_joint_18_source_checkpoint(filename, source_ids, target_ids):
    config = load_experiment_config(_REPO_ROOT / "examples/low_data/configs" / filename)
    suites = ("libero_spatial", "libero_object", "libero_goal")
    assert config.source_task_ids_by_suite() == dict.fromkeys(suites, source_ids)
    assert config.target_task_ids_by_suite() == {
        **dict.fromkeys(suites, target_ids),
